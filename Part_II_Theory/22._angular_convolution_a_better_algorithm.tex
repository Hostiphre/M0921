...
with symmetries eq. (appendix).\textcolor{red}{{} %Cannot make out this sentence. It looks to be in fragments before and after marginpar.
...
The DCF can also be expended on rotational invariants \citep{Blum_I}, %'expended' or 'expanded'?
...
\end{equation}
thus the OZ equation is expended on GSHs and rotational invariants. %Same question about 'expended'.
...
\end{equation}
\marginpar{$\hat{c'}_{\mu\nu,\chi}^{mn}(k)$ is the invariant in eq. (\ref{eq:Blum-reduced-OZ}).}
\begin{equation}
...
Suppose that $F(\mathbf{\Omega})$ is a polynomial of $\cos\Theta$, with
...
or 2 according to the symmetry of axe $\mathrm{C}_{s}$), an overall %Symmetry of 'axe'?
...
For real input function $Y_{k}$ $(k=0,\ldots,n-1)$, FFTW3 only outputs
...
$\mu'$) are done consequentially and \emph{vice versa}. Array index %Not sure if you wanted to say 'consequentially' or 'consecutively'
...
is expended on generalized spherical harmonics %'expended' again.
\begin{equation}
...
\]
where the two can both be expended on GSH %One last time, 'expended' or 'expanded'?
...
And the relation between projections is simple:
