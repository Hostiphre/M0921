...
have transitional and rotational invariance \citep{Blum_I}, it can
be expanded as
\begin{equation}
...
\end{equation}
where $R_{\mu'\mu}^{m}=D_{\mu'\mu}^{m*}$, $D_{\mu'\mu}^{m}$ being
the Wigner generalized spherical harmonics \citep{Edmonds} or Wigner
D-symbol. $f^{mnl}$ can be any arbitrary non-zero constant \citep{Fries_Patey_1985}.
Here we define $f^{mnl}=f^{m}f^{n}=\sqrt{2m+1}\sqrt{2n+1}$ according
to the definition of Belloni \citep{Luc_2014}.

Two special cases are adopted in this thesis, which are the laboratory
...
the same. Here is an example of demonstration with the fixed and local
...
invariant in this two coordinate system.


\section{Transform in local frame}

In the intermolecular (local) coordinate system, the 2 molecules are
both positioned along $z$ axis. Using the properties \citep{Edmonds,Gray-Gubbins,Messiah}
...
and the projections $F_{\mu\nu}^{mnl}(r)$ can be transformed to each
...
\end{equation}
and be transformed with FFT-3D.
...
\end{equation}
which can be treat as a normal FFT of 3 dimension.


\section{Transform in fixed frame}

The same way, in the laboratory coordinate system
\begin{equation}
...
The rotational invariant doesn't take advantage of the $\chi$ transform
...
The total equation of the forward transform is as shown
\begin{equation}
...
\end{equation}
For complex numbers $F_{\eta\nu}^{n}(\mathbf{r})$, the real and imagine
part can be calculated separately.

The rotational matrices $R_{\lambda0}^{l*}(\mathbf{r})$ in Cartesian
coordinate system can be pre-generate by recurrence as detailed in
...
\subsection{Rebuilt of $g(\mathbf{r},\mathbf{\Omega})$ from projections}
...
In IEM and MDFT, the rotational invariants is used to describe the
...
real number property as a physical quantity. Here lists all the symmetric
rules concerns the 2-molecule system.
...
The direct correlation function of bulk solvent has symmetries, as
shown in Figure \ref{fig:Symmetry-operations-of}. In intermolecular
coordinate system, these rules can be easily presented:
\begin{enumerate}
\item Symmetry of vertical mirror: 
\begin{equation}
c(r,\theta_{1},\theta_{2},\phi_{12},\psi_{1},\psi_{2})=c(r,\theta_{1},\theta_{2},-\phi_{12},-\psi_{1},-\psi_{2})
\end{equation}

\item Symmetry of inversion: 
\begin{equation}
c(r,\theta_{1},\theta_{2},\phi_{12},\psi_{1},\psi_{2})=c(r,\pi-\theta_{2},\pi-\theta_{1},\phi_{12},\psi_{2},\psi_{1})
\end{equation}

\end{enumerate}
And the additive symmetry rule for water:
\begin{enumerate}
\item \selectlanguage{english}%
[3.]\selectlanguage{american}%
Symmetry axe $\mathrm{C_{2n}}$: 
\begin{equation}
c(r,\theta_{1},\theta_{2},\phi_{12},\psi_{1},\psi_{2})=c(r,\theta_{1},\theta_{2},\phi_{12},\psi_{1}+\pi,\psi_{2}+\pi)
\end{equation}

\end{enumerate}
The definition of rotational invariant on $\chi$-transform gives
...
In MDFT, rotational invariant projections is used to describe DCF.
\textcolor{red}{The original $c_{\mu\nu}^{mnl}(\mathbf{r})$ collected
by IEM is real.}
\begin{enumerate}
\item With $r$ and $l$: As $c_{\mu\nu}^{mnl}(\mathbf{r})$ is real
\begin{equation}
...
\end{eqnarray}
 is real if $l$ is even, is pure imaginary if $l$ is odd.
\item \selectlanguage{english}%
...
\end{enumerate}
Finally, if the solvent molecule possesses a symmetric axe $\mathrm{C}_{2}$,
$\mu$ and $\nu$ is even.

For spherical solute, the $\mathbf{\Omega_{1}}$-dependence vanishes
so that $m=\mu=0$, only the terms $F_{0\nu}^{0nl}(r)$ is not-zero.


\section{Compare to multipole expansion}
