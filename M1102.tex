...
the idea to use the expansion techniques in \acs{IET} to serve \acs{MDFT}.
The following section \ref{chpt:iem-mdft} gives the extension of
these theories to the molecular solvent case.\\
\medskip{}
Section \ref{chpt:mdft} gives a detailed presentation of the initial
code \acs{MDFT}, which the development of this thesis is based on.}
...
In this thesis, the method is generated to molecular solvent, using %'generated' or 'generalized'?
...
$\rho(\mathbf{r},\mathbf{\Omega})$ is expanded on generalized spherical
harmonics. Theoretically, this approximation gives little loss of
accuracy, but has a great advantage in computing time and memory
requirement. Proof of this is shown in the implementation part.\\
\medskip{}
...
from $O(N^{2})$ to $O(N\log_{2}N)$. A DCF direct issue of Monte
...
is generalized to a molecular case compared to previous work, then interpolation
of zero and first order is involved. To use the projection form, DCF
is reconstructed with all projections. For order of projections $n_{\mathrm{max}}=1$,
the formula of each projection is written explicitly. \\
\medskip{}
...
\include{Part_II_Theory/23._thermodynamic_quantities}

\include{Part_II_Theory/24._solvation_structure}
...
6282 (2.60GHz, 16 cores per processor, with 64 cores per node); 128
GB of memory per node.\\
\medskip{}
The former is used for regular calculation, whose memory hierarchy
is shown in figure \ref{fig:poincare}. The latter is only used in
the evaluation of accuracy, in case of memory leaks in the former.\\
\medskip{}
\begin{figure}[h]
\begin{centering}
...
\medskip{}
Section \ref{chpt:seq-code-performance} evaluates computing performance and
memory limits for the sequential code. Due to the time limit, the
parallelized version has not been totally built, and is included in the
perspective. }
...
\include{Part_IV_Applications/42._solutes}

\ctparttext{The only one publication during this thesis}

\part{Conclusion and Perspectives}

\include{Part_V_Conclusion_and_Perspectives/51._Conclusion}
...
\include{Part_I_State_of_the_Art/1A._computing_performance}\include{Part_I_State_of_the_Art/1B._dcf_of_water}\include{Part_II_Theory/2A._equivalence_of_quadrature_projection_order}\include{Part_II_Theory/2B._rotational_invariant_expansion}

\include{Part_II_Theory/2C._evaluation_of_rotation_matrix_elements_by_recurrence}

\include{Part_II_Theory/2D._symmetry-of-gsh-and-wigner-3j-symbol}\include{Part_II_Theory/2E._error_evaluation_of_interpolation_strategies}

\include{Part_III_Implementation/3A._original_data}
